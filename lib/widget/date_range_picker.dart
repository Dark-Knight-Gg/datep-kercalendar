...
class DateRangePicker extends StatefulWidget {
  DateRangePicker({
    ...
    if (config.calendarType == CalendarDatePickerType.single) {
      assert(initialValue.length < 2,
          'Error: single date picker only allows maximum one initial date');
    }

    if (config.calendarType == CalendarDatePickerType.range &&
  ...
  final RangeDatePickerConfig config;
...
class _DateRangePickerState extends State<DateRangePicker> {
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      if (widget.config.calendarType == CalendarDatePickerType.single) {
        selectedDates = [value];
      } else if (widget.config.calendarType == CalendarDatePickerType.multi) {
        final index =
      ...
      } else if (widget.config.calendarType == CalendarDatePickerType.range) {
      ...
      final isValueDifferent =
          widget.config.calendarType != CalendarDatePickerType.single ||
  ...
  Widget build(BuildContext context) {
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        if (widget.config.calendarType == CalendarDatePickerType.range) {
...
/// [DateRangePicker], or [showDatePicker2] which create full date pickers.
///
/// See also:
///
///  * [DateRangePicker], which provides a Material Design date picker
///    interface.
///
